import { BedrockRuntimeClient, ConverseCommand, SystemContentBlock } from "@aws-sdk/client-bedrock-runtime";
...
async function main() {
  ...
  const largeText = "This is a test of prompt caching demonstrating the benefits of prompt caching. ".repeat(5000);
  ...
  const message1 = {
    role: "user" as const,
    content: [{ text: "Summarize the reference text briefly." }]
  };
  ...
  const message2 = {
    role: "user" as const,
    content: [{ text: "What is the first sentence of the reference text?" }]
  };
